...
def parse_post_times_from_table_like(root: Tag) -> Dict[str, datetime]:
    ...
    for a in root.find_all("a", href=True):
        ...
        if not hhmm:
            ...
            if got: hh,mm,why=got; hhmm,reason=f"{hh:02d}:{mm:02d}", f"next:text/{why}"
        if not hhmm: continue
        hh,mm=map(int, hhmm.split(":"))
        dt=_make_dt_from_hhmm(rid, hh, mm)
        if dt: post_map[rid]=dt
    ...
    def _from_list_page() -> Optional[Tuple[datetime,str,str]]:
        ...
        if not hhmm:
            ...
            if not IGNORE_NEAR_PAT.search(text):
                ...
                if got: hh,mm,why=got; hhmm,reason=f"{hh:02d}:{mm:02d}", f"sibling:text/{why}"
...
def main():
    ...
    logging.info(f"[BUILD] file={p} sha1={sha} v2025-08-13E")
    ...
    try:
        nowH=now_jst().hour
        # 既定：END_HOUR に達してから 0–30分の間で1回呼ばれる想定の常駐/cron
        if nowH >= END_HOUR:
